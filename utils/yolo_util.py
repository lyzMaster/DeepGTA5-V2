...
def gpu_nms(boxes, scores, num_classes, max_boxes=50, score_thresh=0.3, iou_thresh=0.8):
...
def draw_boxes(image, boxes, scores, labels, classes, detection_size,
               font='./files/font/FiraMono-Medium.otf', show=True):
    if boxes is None:
        return image, False
    ...
    for i in range(len(labels)): # for each bounding box, do:
        if labels[i] in det_list:
            ...
            relative = round(((1 - re_width)**4), 2)   # **4宽度变化反映在relative上的效果更加强烈
